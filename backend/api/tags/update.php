...
/**
 * PUT /api/tags/update.php?id=123
 ...
 * tag's slug (nothing currently filters by tag slug — blog listings filter
 * by name — so a rename can't break an existing link); the new name must
 * stay unique.
 */
...
try {
    ...
    if (taxonomy_name_exists($pdo, 'tags', $name, $id)) {
        json_response(409, ['error' => "Tag \"{$name}\" already exists"]);
    }

    try {
        $pdo->prepare('UPDATE tags SET name = :name WHERE id = :id')
    ...
    $updated = taxonomy_find($pdo, 'tags', $id);

    json_response(200, ['data' => taxonomy_format($updated)]);
} catch (Throwable $e) {
    error_log('[tags/update] ' . $e->getMessage());
