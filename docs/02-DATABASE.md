...
- **`blog_posts`** — post content and metadata: `status` (`draft` | `published`), `featured`, `seo_title`/`seo_description` (nullable, fall back to `title`/`description` at read time), `published_at` (set once, on first publish — distinct from `updated_at`), `FULLTEXT` index on `(title, description, content)` for server-side search.
